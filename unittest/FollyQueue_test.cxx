...
dunedaq::iomanager::FollySPSCQueue<int> queue("FollyQueue", 10); ///< Queue instance for the test
...
BOOST_AUTO_TEST_CASE(empty_checks, *boost::unit_test::depends_on("sanity_checks"))
{
  ...
  auto start_time = std::chrono::steady_clock::now();
  BOOST_CHECK_THROW(queue.pop(popped_value, timeout), dunedaq::iomanager::QueueTimeoutExpired);
